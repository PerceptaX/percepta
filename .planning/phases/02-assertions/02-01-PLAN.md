...
Evaluation logic:
- LEDAssertion: Find matching LED signal by name, check state (on/off), color (if specified), blink rate (if specified). Pass if all specified fields match.
- DisplayAssertion: Find matching Display signal by name, exact string match on text content.
- TimingAssertion: Find BootTimingSignal, pass if duration <= MaxDurationMs.
- All: Use confidence from signal for result confidence. If signal not found, fail with confidence 0.0.
  </action>
