...
- SQLite storage driver: Init, Save, Query methods
- Claude Vision driver: Observe(deviceID) returns Observation
- Camera driver: Linux V4L2 capture
...
func Load() (*Config, error) {
    ...
    viper.SetDefault("storage.path", filepath.Join(homeDir, ".percepta", "observations.db"))
...
type Core struct {
    vision  *vision.ClaudeVisionDriver
    storage storage.StorageDriver
}

func NewCore(cameraPath string, dbPath string) (*Core, error) {
    // Initialize vision driver
    visionDriver, err := vision.NewClaudeVisionDriver(cameraPath)
    if err != nil {
        return nil, fmt.Errorf("vision init failed: %w", err)
    }

    // Initialize storage
    storageDriver := storage.NewSQLiteDriver(dbPath)
...
func (c *Core) Observe(deviceID string) (*core.Observation, error) {
    // Capture observation via vision
    obs, err := c.vision.Observe(deviceID)
    if err != nil {
        return nil, err
    }
...
Config loads from ~/.config/percepta/config.yaml with sensible defaults. Core API ties together vision + storage.
  </action>
  <verify>
...
func runObserve(cmd *cobra.Command, args []string) error {
    ...
    // Initialize Core
    perceptaCore, err := percepta.NewCore(cameraPath, cfg.Storage.Path)
    if err != nil {
        return err
    }
    defer perceptaCore.Close()
...
func printObservation(obs *core.Observation) {
    ...
    fmt.Printf("\nStored: ~/.percepta/observations.db\n")
}
```

Command accepts device ID, loads config, initializes Core, captures observation, prints formatted output. Output shows ID, timestamp, each signal with type-specific formatting and confidence.
  </action>
  <verify>
...
- [ ] Observation stored in SQLite at ~/.percepta/observations.db
...
- Core API (pkg/percepta) ties together vision + storage
...
- End-to-end flow: CLI → config → camera → Vision API → parsing → SQLite → output

## Files Created/Modified

- `pkg/percepta/percepta.go` - Public Core API with Observe()
...
- ✅ Cross-platform Go binary (Linux tested)
